...
-- 테이블 저장이 아니라 select 문 저장으로서,  만들어진 뷰를 또다른 select로 조회할때마다
-- 저장된 select 가 실행되며 결과를 보여주집니다.
select * from result_inner_join where job='MANAGER';
-- where 를 붙여서 결과내에서 또다른 조건을 사용할 수 있습니다

-- create or replace view result_inner_join 을 사용하여 다른 select 문을 적용하고 실행하면
-- replace 명령이 동작하여 result_inner_join 라는 이름으로 뷰내용이 대체됩니다.
