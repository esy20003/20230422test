...
public class RentDao {
	...
	public ArrayList<RentDetailDto> selectRentDetail(boolean checkFlag) {
		ArrayList<RentDetailDto> list = new ArrayList<RentDetailDto>();
		con = DBManager.getConnection();
		String sql;
		if (checkFlag) sql = "select * from rentDetail order by numseq desc";
		else sql = "select * from rentDetail order by numseq asc";
		try {
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while( rs.next() ) {
