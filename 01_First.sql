1 -- SQL 파일에서 주석문은 "--"로 표시하고 해당내용을 기술합니다
...
-- 오라클데이터베이스에서 사용가능한 DBMS : SQL Developer, SQL PLUS, 이클립스 등

-- 데이터 베이스의 활용 분야 : 일반웹사이트 게시판의 게시물 저장용, 포터 검색 사이트 검색 대상 정보
...
-- 2. DMS(Daatabase Management Language)
-- 3. DCL(Daatabase Control Language) grant dba to~

-- DDL(Data Defintion Language) - 데이터 정의어
-- 명령의 예 : Create, Alter, Drop
-- Create : 테이블, 뷰 등을 생성할떄 사용하는 명령
-- Alter : 이미 생성되어 있는 테이블 또는 뷰 둥의 구조를 수정하기 위한 명령
...
-- Select : 테이블에 있는 레코드 중 일부 또는 전부를 (조회) 검색하여 열람하기 위한 명령
...
-- 기본키를 지정함으로써 기본키가 지정하지 않았을때 발생할 수 있는 오류들을 방지할 수 있다.
-- 기본키로 결함을 없애는 것. 그래서 결함없이 유지되는 것을 "개체 무결성" 이라고 부릅니다

-- 외래키(FOREIGN KEY) : 테이블 간의 필수 포험 관계에 있어
