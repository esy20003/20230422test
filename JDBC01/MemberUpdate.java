...
class UpdateForm extends JFrame implements ActionListener {
	...
	UpdateForm() {
		...
		// grid 레이아웃 한칸에 두개의 개체를 올리려면 다른 패널에 넣어서 올립니다
		...
		textFieldPanel.add(gender); 
		textFieldPanel.add(joindate); 
		textFieldPanel.add(bpoint); 
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "조회":
			...
			if(mdto == null) {
				System.out.println("해당 회원번호의 회원이 없습니다");
				return;
			} else {
			...
			break;
		case "수정":
			// mdto에 membernum 을 넣겠다 membernum에서 text 를 가져와서 정수형변환해서 
